...
function WorkspaceModal({ modal, setModal, data, defaultAdapterId, adapterList, runAction }: AnyRecord) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (kind === "session") {
      void runAction(() => sendJson("/api/sessions", "POST", { projectId: project.id, agentAdapterId: values.get("agentAdapterId") || defaultAdapterId(), title: values.get("title"), intent: values.get("intent") }), "Session created");
    }
    if (kind === "rule") {
    ...
    <div className="dialog-backdrop">
      <form className="dialog-form dialog-card" onSubmit={submit}>
        ...
        <div className="dialog-fields">
          ...
          {modal.kind === "session" ? <><label>Project<input className="field" value={project?.name || ""} disabled /></label><label>Title<input className="field" name="title" required defaultValue={`Session ${new Date().toLocaleString()}`} /></label><label>Intent<input className="field" name="intent" required placeholder="What should the agent help with?" /></label><label>Agent<select name="agentAdapterId" defaultValue={defaultAdapterId()}>{adapterList.map((adapter: AnyRecord) => <option value={adapter.id} disabled={!adapter.available} key={adapter.id}>{adapter.displayName}{adapter.available ? "" : " (unavailable)"}</option>)}</select></label></> : null}
